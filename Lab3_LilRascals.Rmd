...
```{r}
ggplot(data = cdc)+
  geom_point(aes(x = height,y = weight, color = exerany), position = position_dodge(2))+
  geom_smooth(aes(x = height, y = wtdesire, linetype = gender),color = "red")
```

Team plot: The question we wish to answer is:
Based on the height and weight of an individual, as well as if they exercise. Does their desired weight differ between genders?
The idea behind the question is to see how self confidence in one's body vary between gender with their height and weight in mind. 
Through looking at the data we can see that there is a cluster of data at with varying blue and black dots. The cluster prove that a persons height and weight don't clearly prove that there is a certain body type for each gender. But the line that flows through the cluster show that females weight desired is lower than males which proves that gender influences body confidence. Body confidence isnt influenced by your shape and how much exercise, based on the data females generally desire a smaller weight more than males. 
